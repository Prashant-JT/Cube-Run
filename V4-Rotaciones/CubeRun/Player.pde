public class Player {
  ...
  public float[] getFarFocus() {
    this.focusX = sin(radians(this.angle))*(width*100000);
    this.focusZ = cos(radians(this.angle))*(-width*100000);
  ...
  public int [] getBoundsX() {
    float[] pos = getPos();
    return new int[]{int(pos[0])-5, int(pos[1])+5}; 
  }
  
  public int [] getBoundsY() {
    float[] pos = getPos();
    return new int[]{int(pos[2])-1, int(pos[3])-1};
  }
  
  public int [] getBoundsZ() {
    float[] pos = getPos();
    return new int[]{int(pos[4])-5, int(pos[5])+5};
  }
}
