public class Player {
  ...
  public float[] getPos(){
    ...
    return new float[]{minX - 20, minY - 20, minZ - 20};
  }  
  
  public void updateCamera() {
    float [] pos = getPos();  
    camera(pos[0] + 27, pos[1] + 80, pos[2] + 200,
            0, -150, -100000,
            0.0, -0.5, 1.0);
            
    /*camera(pos[0] -50, pos[1] + 80, pos[2] + 200,
           50000, -150, -100000,
           0.0, -0.5, 0.0);*/
  }
  ...
  public int [] getBoundsX() {
    float[] pos = getPos();
    return new int[]{int(pos[0]), int(pos[0]) - 20}; 
  }
  
  public int [] getBoundsY() {
    float[] pos = getPos();
    return new int[]{int(pos[1]), int(pos[1]) + 20};
  }
  
  public int [] getBoundsZ() {
    float[] pos = getPos();
    return new int[]{int(pos[2]), int(pos[2]) - 20};
  }
}
